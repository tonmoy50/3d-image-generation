...
def change_rgba_bg(rgba: Image.Image, bkgd="WHITE"):
    ...
    new_rgba = Image.fromarray(
        np.concatenate([np.array(rgb_white), np.array(rgba)[:, :, 3:4]], axis=-1)
    )
    return new_rgba
...
def make_image_grid(images, rows=None, cols=None, resize=None):
    ...
    if total_imgs > len(images):
        images += [
            Image.new(images[0].mode, images[0].size)
            for _ in range(total_imgs - len(images))
        ]
